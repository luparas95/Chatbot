...
def get_obj(db, value, by_id=False):
    if by_id:
        data = db.execute(1, 'SELECT * FROM Nationality WHERE Id = ?', (value,))
    else:
        data = db.execute(1, 'SELECT * FROM Nationality WHERE Name = ?', (value,))
...
def get_nb_rows(db):
    return db.execute(1, 'SELECT COUNT(*) FROM Nationality')


def get_all_rows(db):
    return db.execute(2, 'SELECT * FROM Nationality')
...
class Nationality:
    ...
    def insert(self, db, user):
        if get_obj_role_permission(db, [user.get_role, "Insert"]) is not None:
            if self.__is_valid(db):
                ...
                db.execute(
                    3, 'INSERT INTO Nationality VALUES (NULL, ?, ?, current_timestamp, ?, current_timestamp)',
    ...
    def delete(self, db, user):
        if get_obj_role_permission(db, [user.get_role, "Delete"]) is not None:
            if self.__exist(db):
                db.execute(3, 'DELETE FROM Nationality WHERE Id = ?', self.get_id())
