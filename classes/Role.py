...
def get_obj(db, value, by_id=False):
    if by_id:
        data = db.execute(1, 'SELECT * FROM Role WHERE Id = ?', (value,))
    else:
        data = db.execute(1, 'SELECT * FROM Role WHERE Name = ?', (value,))
...
def get_nb_rows(db):
    return db.execute(1, 'SELECT COUNT(*) FROM Role')


def get_all_rows(db):
    return db.execute(2, 'SELECT * FROM Role')
...
class Role:
    ...
    def insert(self, db, user):
        if RolePermission.get_obj(db, [user.get_role, "Insert"]) is not None:
            if self.__is_valid(db):
                ...
                db.execute(
                    3, 'INSERT INTO Role VALUES (NULL, ?, ?, current_timestamp, ?, current_timestamp)',
    ...
    def delete(self, db, user):
        if RolePermission.get_obj(db, [user.get_role, "Delete"]) is not None:
            if self.__exist(db):
                db.execute(3, 'DELETE FROM Role WHERE Id = ?', self.get_id())
