...
def get_obj(db, value, by_id=False):
    if by_id:
        data = db.execute('SELECT * FROM Permission WHERE Id = ?', (value,), True)
    else:
        data = db.execute('SELECT * FROM Permission WHERE Name = ?', (value,), True)
...
def get_nb_rows(db):
    return db.get_nb_rows('SELECT COUNT(*) FROM Permission')


def get_all_rows(db):
    return db.get_all_rows('SELECT * FROM Permission')
...
class Permission:
    ...
    def insert(self, db, user):
        if RolePermission.get_obj(db, [user.get_role, "Insert"]) is not None:
            if self.__is_valid(db):
                ...
                db.execute(
                    'INSERT INTO Permission VALUES (NULL, ?, ?, current_timestamp, ?, current_timestamp)',
    ...
    def delete(self, db, user):
        if RolePermission.get_obj(db, [user.get_role, "Delete"]) is not None:
            if self.__exist(db):
                db.execute('DELETE FROM Permission WHERE Id = ?', self.get_id())
